...
기존 프로젝트에서 퀴즈의 시간당 시작 횟수 제한만 제거하려면 SQL Editor에서
`supabase-remove-quiz-rate-limit.sql`을 한 번 실행합니다. 새 프로젝트는 최신
`supabase-setup.sql`에 이미 같은 변경이 포함되어 있으므로 별도 실행이 필요 없습니다.
